...
gcloud compute ssh "$INSTANCE_NAME" \
  --project="$PROJECT_ID" \
  --zone="$ZONE" \
  --command="sudo -u $VM_USER bash -lc 'cd /opt/stock-scanner-cloud && git fetch origin && git checkout $BRANCH && git pull --ff-only origin $BRANCH && python3 -m venv .venv && .venv/bin/pip install -r requirements.txt && chmod +x ibkr_bridge/scripts/start_ibkr_gateway.sh' \
