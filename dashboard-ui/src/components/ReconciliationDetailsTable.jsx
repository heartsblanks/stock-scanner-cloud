          ...
          <tbody>
            {sortedRows.map((row, index) => (
              <tr key={`${row.broker_parent_order_id || "row"}-${index}`} className={toneForMismatch(row)}>
                ...
                <td data-label="Live Entry Price">{formatCurrency(row.alpaca_entry_price)}</td>
                ...
                <td data-label="Live Exit Price">{formatCurrency(row.alpaca_exit_price)}</td>
                ...
                <td data-label="Live Entry Qty">{formatNumber(row.alpaca_entry_qty, 0)}</td>
                <td data-label="Live Exit Qty">{formatNumber(row.alpaca_exit_qty, 0)}</td>
                <td data-label="Local Exit Reason">{formatValue(row.local_exit_reason)}</td>
                <td data-label="Live Exit Reason">{formatValue(row.alpaca_exit_reason)}</td>
                <td data-label="Live Exit Order ID" className="dashboard-cell-muted">{formatValue(row.alpaca_exit_order_id)}</td>
              </tr>
            ))}
          </tbody>
        </table>
