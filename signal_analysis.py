...
def build_signal_rows(signal_rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for row in signal_rows:
        ...
        rows.append({
            ...
            "skipped_count": max(candidate_count - placed_count, 0),
...
def run_signal_analysis() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    raw_signal_rows = get_gcs_csv_rows(SIGNALS_BUCKET_NAME, SIGNALS_OBJECT)
...
def main() -> None:
    ...
    print(f"\nWrote signal summary CSV to {ANALYSIS_SUMMARY_OUTPUT}")
    print(f"Wrote signal rows CSV to {ANALYSIS_SIGNAL_ROWS_OUTPUT}")
    print(f"Signal rows analyzed: {len(signal_rows)}")
